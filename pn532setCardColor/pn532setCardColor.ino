...
#define cardColor    1  //card color red : 1, blue : 2, yellow : 3

uint8_t redBlock[] = {0x0A, 0x11, 0x00, 0x00};
uint8_t blueBlock[] = {0x0B, 0x00, 0x11, 0x00};                 
uint8_t yellowBlock[] = {0x0C, 0x00, 0x00, 0x11};
...
void loop(void) {
  ...
  if (success)
  {
    ...
    if (uidLength != 4)
    {
      ...
      success = nfc.mifareReadDataBlock(4, data);
      if(!success){
        Serial.println("it is not working reading block 4");
        return;
      }
